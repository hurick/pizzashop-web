...
test('display monthly orders amount metric', async ({ page }) => {
  ...
  await expect(page.getByText('+18% em relação ao mês')).toBeVisible()
})
...
test('display monthly revenue', async ({ page }) => {
  ...
  await expect(page.getByText('R$ 123,11', { exact: true })).toBeVisible()
  await expect(page.getByText('+21% em relação ao mês')).toBeVisible()
})
